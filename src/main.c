...
** Last update Tue Mar 28 18:50:46 2017 linder
*/
...
static char	init_player_map(t_player ***players)
{
  int		x;
  int		y;

  if ((*players = malloc(sizeof(t_player) * MAP_HEIGHT)) == NULL)
    return (exit_error("Error malloc", 84));
  if (*players == NULL)
    return 0;
  y = -1;
  while (++y < MAP_HEIGHT)
    {
      if (((*players)[y] = malloc(sizeof(t_player) * MAP_WIDTH)) == NULL)
	return (exit_error("Error malloc", 84));
      x = -1;
      while (++x < MAP_WIDTH)
	(*players)[y][x] = 0;
...
static char	start(char *path, int team_nb)
{
  ...
  if (shm_key == -1)
    return (exit_error(NULL, 84));
  if ((s_ipc_shared = (t_ipc_shared*)shmat(shm_key, NULL, SHM_R | SHM_W)) == -1)
    return (exit_error(NULL, 84));
  return (0);
}
